...
class GossipDataParallel(Module):
    ...
    def __init__(self, module, device_ids=None, rank=None, world_size=None,
                 graph=None, comm_device=None, lr=0.1, 
        ...
        self.dist_config = {
            ...
            'epoch':            0.0,
            'iterations':       0,
            'n':                torch.ones(1, device=comm_device).type(first_param_dtype),}
        ...
        self.gossiper   =     Relay(msg        =  torch.cat([flatten_tensors(self.send_params), self.dist_config['n']]),
    ...
    def transfer_params(self, mix=True, epoch=0, lr=0.1):
        ...
        with self.gossip_lock:
            in_msg, updated_n, data_amt = self.gossiper.mix(send_msg)
            self.dist_config['n'] = updated_n
    ...
    def gossip_averaging(self):
        ...
        for p, r in zip(self.module.parameters(), self.gossip_device_buffer):
            p.data.add_(r.data)  
            p.data.div_(self.dist_config['n'])
